...
STCUK.utility = {
	...
	getHeaderHeight: function (e) {
		var t = $('.ui-component.header'),
			a = $('.cookie-container'),
			n = t.height() || 0,
			r = 0;
		return e && a.length && (r = a.height()), n + r;
	},
					...
					var n = t.errorMessages,
						...
						o = $(
							'<li class="form-error-item">' +
								...
								' <a title="' +
								i +
								'" href="javascript:void(0)">Click to see error(s)</a></li>'
						);
	...
	cookieMsgShowing: function () {
		return !document.querySelector('[data-role="cookie"]').classList.contains('hide');
	},
	cookie: {
		getItem: function (e) {
			return (
	...
	applePay: {
		...
		setDomElements: function (e) {
			(this.donationMethodInstance = e),
				...
				(this.sandboxMode = 'true' === this.donationMethodInstance.$applePayBtn.attr('data-sandbox-mode')),
		...
		_setDomElements: function (e) {
			(this.payingWithApplePay = !1),
				...
				'true' === e.attr('data-sandbox-mode') && (this.sandboxMode = !0);
		},
		applePayCapableCheck: function (e, a, t) {
			var n = STCUK.payments.applePay;
			window.ApplePaySession
				? ApplePaySession.canMakePaymentsWithActiveCard(n.applePayMerchantId).then(function (e) {
						(n.canUserMakePayments = !1), a && a.setDebitAsDefault();
				  })
					...
					s.completePayment(ApplePaySession.STATUS_SUCCESS),
						o.sandboxMode &&
							(a.token.paymentData = {
								...
								signature: 'bbb',
								header: { transactionId: 'ccc', ephemeralPublicKey: 'ddd', publicKeyHash: 'eee' }
							}),
		...
		populateAddressDetails: function (e) {
			...
			_.each(p, function (e) {
				...
				n.find(d).val(e.billingContact.postalCode),
				a.skipToLastPanel();
		},
...
STCUK.deviceProfile = {
	...
	setDomElements: function (e) {
		...
		(this.$el = i),
			...
			(this.cookieMsg = document.querySelector('[data-role="cookie"]')),
			(this.cookieMsgClass = null),
			(this.$mobLogo = this.$headerWrapper.find('.visible-xs'));
	},
	closePopup: function () {
		var e = STCUK.deviceProfile;
		e.$helpPopup.addClass('hidden'),
			e.$headerWrapper.removeClass('not-fixed'),
			e.cookieMsgClass &&
				(e.$commerceHeaderLinks.css('margin-top', 0),
				document.documentElement.classList.remove(e.cookieMsgClass),
				(this.cookieMsgClass = null)),
			STCUK.captcha.hidePanel();
	},
	cookieClosed: function () {
		var e = STCUK.deviceProfile;
		e.$commerceHeaderLinks && e.$commerceHeaderLinks.css('margin-top', 0);
	},
	...
	getDeviceProfile: function () {
		var e = STCUK.deviceProfile,
			...
			a = '',
			t = '',
			n = window.location.pathname + window.location.search + window.location.hash,
			o = STCUK.regexes.devices,
			r = STCUK.regexes.os,
			d = STCUK.regexes.browsers;
		if (((e.sendPayload.deviceDetails = {}), void 0 !== i))
			for (var l in i)
				null !== l.match(o) && (s = l + ' ' + i[l]),
					null !== l.match(r) && (a = l + ' ' + i[l]),
					null !== l.match(d) && (t = l + ' ' + i[l]);
		var c = e.formatString(s);
		e.$deviceField.text(c), (e.sendPayload.deviceDetails.device = c);
		var p =
			t.indexOf('fbav') > -1 || t.indexOf('fbsv') > -1
				? t.replace(/fb(a|s)v/, 'Facebook app')
				: e.formatString(t);
		e.$browserField.text(p), (e.sendPayload.deviceDetails.browser = p);
		var m = e.formatString(a);
		e.$OSfield.text(m), (e.sendPayload.deviceDetails.os = m);
		var f = e.getCurrPage(n);
		e.$currPageField.text(f), (e.sendPayload.page = f);
	},
	showHelpPopup: function (e) {
		var i = STCUK.deviceProfile,
			s = new Date(),
			a = STCUK.utility,
			t = a.checkMobileResolution();
		if (
			a.cookieMsgShowing() &&
			((i.cookieMsgClass = t ? 'rossbot-cookie-msg-mob' : 'rossbot-cookie-msg-desktop'),
			document.documentElement.classList.add(i.cookieMsgClass),
			t)
		) {
			var n = STCUK.cookie.getCookieMsgHeight();
			i.$commerceHeaderLinks && i.$commerceHeaderLinks.css('margin-top', n + 'px');
		}
		i.$sendDetailsForm.parsley().reset(),
			...
			i.$helpPopup.hasClass('hidden')
				? ((i.sendPayload.date = s.toGMTString()), i.$helpPopup.removeClass('hidden'), i.populateDetails())
				: i.closePopup(),
	...
	populateDetails: function () {
		var e = STCUK.deviceProfile,
			...
			s = $('input[name=surname]').val(),
			a = e.getDISession();
		if (i && s) {
			var t = i + ' ' + s;
			new RegExp(STCUK.regexes.nameWithAccents).test(t) ? e.$sendDetailsName.val(t) : e.$sendDetailsName.val('');
		}
		var n = $('input[name=email_address]').val();
		n &&
			'' !== n &&
			(new RegExp(STCUK.regexes.email).test(n) ? e.$sendDetailsEmail.val(n) : e.$sendDetailsEmail.val(''));
		'string' == typeof a &&
			a.length > 0 &&
			(new RegExp(/^di-/).test(a) ? e.$sendDetailsDISessionId.val(a) : e.$sendDetailsDISessionId.val(''));
	},
	...
	sendHelpFormPayload: function (e) {
		var i = STCUK.deviceProfile;
		STCUK.captcha.fetchToken(function (s) {
			$.ajax({
				url: '/deviceProfile',
				method: 'GET',
				dataType: 'text',
				data: { token: s, deviceProfile: JSON.stringify(e) }
			})
				.success(function () {
	...
	parsleyValidator: function (e) {
		var i = $(e);
		_.each(i.find('input'), function (e, i, s) {
...
STCUK.actionFormIFrame = {
	...
	sendFormData: function () {
		...
		sessionStorage.getItem('firstName') &&
			...
			e.contentWindow.postMessage(a, t);
	},
	setDomElements: function (e, t) {
		var a = STCUK[t];
		(a.invocationName = t), (a.$el = e);
		var o = STCUK.analytics;
		a.iframe = e.querySelector('iframe');
		var r = o.cidTracking().marketingSource;
		(a.source = e.dataset.iframeSource + ('' !== r ? '?sourcecode=' + r : '')), a.addSrcAttribute();
	},
	...
	init: function (e, t) {
		var a = STCUK[t];
		a.bindEvents(t), a.setDomElements(e, t);
	}
};
STCUK.applePay = {
	bindEvents: function () {
	...
	setDomElements: function (e, a) {
		...
		(e.dataset.invocationName = a),
			...
			(STCUK.payments.applePay.sandboxMode = 'true' === e.getAttribute('data-sandbox-mode')),
...
STCUK.cookie = {
	bindEvents: function () {
		var e = STCUK.cookie;
		e.$cookieCloseBtn.addEventListener('click', e.onCloseCookie.bind(e));
	},
	setDomElements: function (e) {
		(this.$el = e),
			...
			(this.$cookieCloseBtn = e.querySelector('.cookie-close-icon'));
	},
	cookieExpiryDate: function (e) {
		var o = new Date();
		return o.setMonth(o.getMonth() + e), o;
	},
	onCloseCookie: function () {
		var e = STCUK.cookie,
			o = e.cookieExpiryDate(3);
		e.$cookie.classList.add('hide'),
			STCUK.utility.cookie.setItem('use-cookie-agree', 'true', o, '/'),
			e.$body.classList.remove('cookie-visible'),
			STCUK.deviceProfile.cookieClosed();
	},
	getCookieMsgHeight: function () {
		return STCUK.cookie.$el.clientHeight;
	},
	init: function (e) {
		var o = STCUK.cookie,
			i = STCUK.utility;
		o.setDomElements(e);
		var t = i.cookie.getItem('use-cookie-agree') || '';
		t && 'true' === t
			? (o.$cookie.classList.add('hide'), o.$body.classList.remove('cookie-visible'))
			: (o.$cookie.classList.remove('hide'),
			  o.$cookieContainer.classList.remove('hide'),
			  o.$body.classList.add('cookie-visible')),
			o.bindEvents();
	}
};
STCUK.donateOverlap = {
...
STCUK.donatePod = {
	bindEvents: function () {
		...
		e.$giftAidLinks.forEach($link => {
			$link.addEventListener('click', STCUK.donatePod.launchGiftAidTooltip.bind(e));
		});

		e.$closeGiftAidTextBtn.addEventListener('click', e.hideGiftAidTooltip.bind(e));
	},
	setDomElements: function (e, t) {
		var n = STCUK[t];
		(n.$el = e),
		...
		n.$giftAidLinks = n.$el.querySelectorAll('.gift-aid-read-more-btn');
		n.$giftAidWrapper = n.$el.querySelector('.gift-aid-explanation');
		n.$closeGiftAidTextBtn = n.$el.querySelector('.gift-aid-explanation-close-btn');
	},
	toggleFade: function (e) {
		var t = STCUK[this.invocationName],
			n = e.currentTarget,
			a = n.closest('.donate-pod-panel'),
			o = a.querySelector('.donate-pod-amount-container'),
			i = t.getActiveDescription(a);
		'focus' === e.type && (o.classList.add('faded'), i.classList.add('faded')),
			'blur' === e.type && '' === n.value && (o.classList.remove('faded'), i.classList.remove('faded'));
	},
	toggleAmount: function (e) {
		...
		'single' === t.getDonationType(n.closest('.donate-pod-panel'))
			? (t.$selectedSingleAmount.classList.remove('active'),
			  ...
			  (a = t.getNewDescription(t.$singlePanel, n.getAttribute('data-btn-index'))),
			  t.$selectedSingleDescription.classList.add('hidden'),
			  (t.$selectedSingleDescription = a))
			: (t.$selectedMonthlyAmount.classList.remove('active'),
			  (t.$selectedMonthlyAmount = n),
			  (a = t.getNewDescription(t.$monthlyPanel, n.getAttribute('data-btn-index'))),
			  t.$selectedMonthlyDescription.classList.add('hidden'),
			  (t.$selectedMonthlyDescription = a)),
	...
	clearOtherAmount: function (e) {
		var t = STCUK[this.invocationName],
			...
			a = n.querySelector('.other-amount-input'),
			o = n.querySelector('.other-amount-wrapper .error-message'),
			i = t.getPanelState().activePanel,
			r = t.getActiveDescription(i),
			l = n.querySelector('.donate-pod-amount-container');
		(a.value = ''), a.blur(), l.classList.remove('faded'), r.classList.remove('faded'), t.hideValidationErr(a, o);
	},
	...
	checkOtherFieldIsValid: function (e) {
		var t = STCUK[this.invocationName],
			n = e.target,
			a = n.checkValidity(),
			o = n.closest('.other-amount-wrapper').querySelector('.error-message');
		!1 === a
			? (t.singleDefaultErrText && (t.$singleErrorMessage.textContent = t.singleDefaultErrText),
			  ...
			  t.showValidationErr(n, o))
			: t.hideValidationErr(n, o);
	},
	...
	positionOverlayPanel: function (e, t) {
		var n = STCUK[this.invocationName],
			...
			o = n.$panelTabs[0].offsetHeight;
		n.isDesktopDimensions()
			? ((t.style.width = Math.ceil(a.width) + 'px'),
	...
	launchDonationMethodOverlay: function (e) {
		var t = STCUK[this.invocationName],
			n = t.donationDetails,
			a = t.getPanelState().activePanel,
			o = t.donationType,
			i = t.getActiveDonateMethodOverlay(a),
			r = 'single' === o ? t.$singleDonatingHeading : t.$monthlyDonatingHeading,
			l = 'single' === o ? t.singleDonatingHeadingInitVal : t.monthlyDonatingHeadingInitVal,
			s = 'single' === o ? t.$singleGiftAidLabel : t.$monthlyGiftAidLabel,
			c = 'single' === o ? t.$singleGiftAidLabelInitialVal : t.$monthlyGiftAidLabelInitialVal;
		'monthly' === o && t.$payPalDatePicker && t.populatePaypalRGStartDate(), t.positionOverlayPanel(a, i);
		var d = STCUK.utility.calculateGiftAid(n.amount);
		if (((t.donationDetails.giftAidVal = d), n.currencyVal && n.amount)) {
			var y = 0 === n.currencyIndex ? '' + n.currencyVal + n.amount : '' + n.amount + n.currencyVal,
				u = 0 === n.currencyIndex ? '' + n.currencyVal + d : '' + d + n.currencyVal;
			(r.textContent = l + ' ' + y),
				(s.textContent = c.replace(/{\w+}/, u)),
				t.$panelTabsContainer.classList.add('disabled'),
				i.classList.remove('hidden'),
				setTimeout(function () {
					t.setVerticalOffset(i), t.toggleBackgroundScroll(!1), i.classList.add('open');
				}, 100);
		}
	},
	...
	handleDonateBtnClick: function (e) {
		var t = STCUK[this.invocationName],
			n = t.getPanelState(),
			a = n.activePanel,
			o = 'true' === a.getAttribute('data-donate-method-overlay');
		if (
			((t.formUrl = a.dataset.buttonUrl),
			(t.donationType = t.getDonationType(a)),
	...
	checkValIsValid: function (e, t) {
		...
		if (e) {
			var o = n.checkMinValue(e, a, t);
			return (
				!o ||
				((o.errMsgEl.textContent = o.errorMsg),
				n.showValidationErr(o.errField, o.errMsgEl),
				n.showErrorFlicker(o.errField, o.errMsgEl, 500),
				!1)
			);
		}
		return (
			'single' === t
				...
				: ((n.$monthlyErrorMessage.textContent = n.monthlyDefaultErrText),
	...
	checkMinValue: function (e, t, n) {
		...
		return (
			...
			('single' === n
				? ((o = a.$singleErrorMessage), (i = a.$singleOtherAmount))
				: ((o = a.$monthlyErrorMessage), (i = a.$monthlyOtherAmount)),
			{ errorMsg: 'The minimum donation value is ' + a.parseValue(e).currencyVal + t, errMsgEl: o, errField: i })
		);
	},
	showErrorFlicker: function (e, t, n) {
		var a = STCUK.utility;
		a.flicker(e, n), a.flicker(t, n);
	},
	getDonatePodValue: function () {
		var e = STCUK[this.invocationName],
			t = e.donationType,
			n = 'single' === t ? e.$singleErrorMessage : e.$monthlyErrorMessage,
			a = 'single' === t ? e.$singleOtherAmount : e.$monthlyOtherAmount;
		if (n.classList.contains('hidden'))
			return (e.isCustomAmt = '' !== a.value), e.isCustomAmt ? a.value : e.getActiveAmount(t);
	},
	parseValue: function (e) {
		var t = STCUK[this.invocationName],
			n = { currencyVal: '£', currency: 'GBP', amount: '0', currencyIndex: 0 },
			a = e.match(t.currencyRegex);
		if (null !== a) {
			switch (a[0]) {
				case '$':
			...
			(n.amount = e.replace(a[0], '')), (n.currencyIndex = a.index ? a.index : 0);
		} else n.amount = e;
		return n;
	},
	redirectToForm: function () {
		...
		window.sessionStorage && Modernizr.sessionstorage
			...
			: (a.cookie.setItem('donationAmount', t.amount, null, '/'),
			  a.cookie.setItem('donationType', e.donationType, null, '/'),
			  a.cookie.setItem('currency', t.currency, null, '/'),
			  a.cookie.setItem('currencyVal', t.currencyVal, null, '/'),
			  a.cookie.setItem('currencyText', e.donatePodValue, null, '/')),
	...
	submitFormPost: function (e) {
		...
		STCUK.captcha.fetchToken(function (a) {
			(e.formdata.form_attributes.token = a),
				fetch(t.actionUrl, {
					method: 'post',
					...
					.catch(function (n) {
						console.error(n);
						var a = e.formdata.donation_method_acc.donation_method_rbg;
						'apple' === a && STCUK.applePay.enableApplePayBtn(), t.handleFormsEngineError(a);
					});
		});
	},
	handleFormsEngineError: function (e) {
		var t = arguments.length > 1 && void 0 !== arguments[1] && arguments[1],
			...
			o = 'paypal' === e ? 'Paypal' : 'Apple pay',
			i = 'paypal' === e ? 'Paypal account' : 'Apple pay wallet';
		STCUK.loader.hide();
		var r = 'single' === a.donationType ? a.$singleDonateOverlay : a.$monthlyDonateOverlay,
			l = 'single' === a.donationType ? a.$singleFormsEngineError : a.$monthlyFormsEngineError,
			s = parseInt(l.dataset.paymentErrorCount);
		t &&
			(a.$panelTabs.forEach(function (e) {
				e.classList.contains(a.donationType + '-tab') && e.click();
			}),
			(s = parseInt(n)));
		var c = l.querySelector('h2'),
			d = c.textContent;
		c.textContent = d.replace(/{\w+}/, o);
		var y = l.querySelectorAll('.error-msg-text');
		if (
			(y.forEach(function (e) {
				var t = e.textContent;
				e.textContent = t.replace(/{\w+}/, i);
			}),
			a.showCorrectErrorMsg(l, s),
			r.classList.add('reduce-z-index'),
			l.classList.remove('hidden'),
			l.classList.add('open'),
			t)
		) {
			var u = a.getPanelState(),
				m = u.activePanel.querySelector('.donate-btn');
			m.click();
		}
	},
	...
	closeErrorPanel: function (e) {
		var t = STCUK[this.invocationName],
			...
			a = 'single' === t.donationType ? t.$singleFormsEngineError : t.$monthlyFormsEngineError;
	...
	launchGiftAidTooltip: function (e) {
		var thisComponent = STCUK[this.invocationName];
		const $btn = e.target;
		...
		// const buttonMargin = buttonStyles.marginTop.replace('px', '');
		const halfButtonHeight = buttonPos.height / 2;
		// const yStartPoint = buttonOffsetTop - buttonMargin + halfButtonHeight;
		// const halfGiftAidPanelHeight = thisComponent.$giftAidWrapper.clientHeight / 2;
		// const yOffset = yStartPoint - halfGiftAidPanelHeight;
		thisComponent.$giftAidWrapper.classList.remove('hidden');
		thisComponent.$giftAidWrapper.style.left = `${Math.ceil(buttonPos.right)}px`;
		thisComponent.$giftAidWrapper.style.top = `${buttonOffsetTop + halfButtonHeight}px`;
		console.log(buttonPos);
	},

	hideGiftAidTooltip: function (e) {
		var thisComponent = STCUK[this.invocationName];
		var $giftAidPanel = e.target.closest('.gift-aid-explanation');
		$giftAidPanel.classList.remove('open');
	},
...
STCUK.heroImage = {
	...
	setDomElements: function (e, o) {
		(this.$el = e),
			...
			(this.$cookieContainer = document.querySelector('.ui-component.cookie-container')),
...
(STCUK.map = {
	...
	setDomElements: function (e, t) {
		...
		(e.dataset.invocationName = t),
			(o.invocationName = t),
			(o.$el = e),
			(o.$map = e.querySelector('.map')),
			(o.$locator = e.querySelector('.locator')),
			(o.$results = e.querySelector('.results')),
			(o.isIe = void 0 !== cssua.ua.ie);
		var a = e.querySelector('script');
		(o.config = JSON.parse(a.innerText.trim())),
			(o.dataSources = []),
			o.$locator &&
				((o.$searchInput = o.$locator.querySelector('input')),
				navigator.geolocation &&
					!o.isIe &&
					((o.$geolocationBtn = o.$locator.querySelector('.use-location')),
					o.$geolocationBtn.classList.remove('hide')),
				(o.$searchBtn = o.$locator.querySelector('.search-btn')),
				(o.$clearBtn = o.$locator.querySelector('.clear-btn'))),
			(o.searchResultsAmount = +e.dataset.searchResultsAmount),
			(o.tileSize = { height: 256, width: 256 }),
			(o.zoomMax = 21),
			(o.buffer = 100),
			(o.mapDimensions = {}),
			(o.mapOffset = { x: 0, y: 0 }),
			(o.$loaderMessagePanel = e.querySelector('.map-loader-message')),
			(o.$loaderMessage = e.querySelector('.loading-message')),
			(o.$resultsMessage = o.$locator.querySelector('.results-message')),
			(o.$resultsMessageText = o.$resultsMessage.textContent),
			(o.loadMoreShopsIncrementAmount = +o.$locator.dataset.showIncrementAmount),
			(o.loadMoreShops = o.$locator.dataset.showMore),
			(o.$locator.dataset.shopCount = '0'),
			(o.shopLatLng = {}),
			(o.addMoreShopsClicked = !1),
			(o.firstShopLatLng = {}),
			(o.searchResultsMessage = o.$locator.querySelector('.results-message').dataset.searchResultsMessage),
			(o.searchErrorMessage = o.$locator.querySelector('.results-message').dataset.searchErrorMessage);
	},
	...
	loadDataSources: function () {
		...
		e.config.forEach(function (t) {
			for (var o = t.dataSource, a = !1, s = 0; !a; ) {
				var n = '' + o + s;
				STCUK[n] ||
					((STCUK[n] = _.clone(STCUK[o])), STCUK[n].init(e, n, t), (a = !0), e.dataSources.push(STCUK[n])),
					s++;
			}
		});
	},
	...
	zoomLatLng: function (e, t, o) {
		return Math.floor(Math.log(e / t / o) / Math.LN2);
	},
	latRadian: function (e) {
		var t = Math.sin((e * Math.PI) / 180),
			o = Math.log((1 + t) / (1 - t)) / 2;
		return Math.max(Math.min(o, Math.PI), -Math.PI) / 2;
	},
	getBoundsZoomLevel: function (e, t) {
		var o = STCUK[this.invocationName],
			a = e.getNorthEast(),
			s = e.getSouthWest(),
			n = (o.latRadian(a.lat()) - o.latRadian(s.lat())) / Math.PI,
			r = a.lng() - s.lng(),
			i = (r < 0 ? r + 360 : r) / 360,
			l = o.zoomLatLng(t.height, o.tileSize.height, n),
			c = o.zoomLatLng(t.width, o.tileSize.width, i);
		return Math.min(l, c, o.zoomMax);
	},
	getBounds: function (e) {
		...
		var t = void 0,
			...
			s = void 0;
		e.forEach(function (e) {
			if (!t) return (t = a = e.lat), void (s = o = e.lng);
			e.lat > t ? (t = e.lat) : e.lat < a && (a = e.lat), e.lng < o ? (o = e.lng) : e.lng > s && (s = e.lng);
		});
	...
	zoomWithOffset: function (e) {
		...
		e ? (t.map.setZoom(a), t.map.panTo(n)) : (t.map.setCenter(n), t.map.setZoom(a));
	},
	offsetLatLng: function (e, t, o) {
		var a = STCUK[this.invocationName];
		(t = t || 0), (o = o || 0);
		var s = Math.pow(2, a.map.getZoom()),
			n = a.map.getProjection().fromLatLngToPoint(e),
			r = new google.maps.Point(t / s, o / s),
	...
	setOffsetCentre: function (e) {
		...
		var o = t.offsetLatLng(e, t.mapOffset.x / 2, t.mapOffset.y / 2);
		t.map.panTo(o);
	},
	zoom: function (e) {
	...
	setBounds: function (e, t) {
		var o = STCUK[this.invocationName];
		o.updateMapDimensions();
		var a = o.getBounds(e),
			s = {
				width: o.mapDimensions.width - o.mapOffset.x - 2 * o.buffer,
				height: o.mapDimensions.height - o.mapOffset.y - 2 * o.buffer
			},
			n = o.getBoundsZoomLevel(a, s);
		t ? o.map.setZoom(n / 2) : o.map.setZoom(n), o.setOffsetCentre(a.getCenter());
	},
	handleSearchBtn: function () {
		var e = document.querySelector('.pac-target-input');
		function t() {}
		google.maps.event.trigger(e, 'focus', {}),
			google.maps.event.trigger(e, 'keydown', { keyCode: 40, stopPropagation: t, preventDefault: t }),
			google.maps.event.trigger(e, 'keydown', { keyCode: 13 }),
	...
	markerSearch: function (e, t) {
		var o = STCUK[this.invocationName];
		document.querySelector('.results').classList.contains('hide') && o.search(e, t, !0);
	},
	toggleResultsMessage: function (e, t, o) {
		var a = STCUK[this.invocationName],
			s = (a.$resultsMessage.textContent = a.$resultsMessageText.replace(
				/x|\d{1,3}/gi,
				a.$locator.dataset.shopCount
			)),
			n = a.$loaderMessagePanel.classList.contains('hide');
		(a.$resultsMessage.textContent = o ? a.searchErrorMessage : s),
			t && (n || a.$loaderMessagePanel.classList.add('hide'), e || a.$results.classList.add('top-no-message')),
			e
				? (a.$resultsMessage.classList.remove('hide'),
				  a.$results.classList.remove('top-no-message'),
				  a.$results.classList.add('top-message'))
				: a.$resultsMessage.classList.add('hide');
	},
	setDeviceMarkerBounds: function (e, t, o) {
		var a = STCUK[this.invocationName];
		if (STCUK.utility.checkMobileResolution()) o ? a.map.setZoom(15) : a.map.fitBounds(e);
		else {
	...
	search: function (e, t, o) {
		var a = STCUK[this.invocationName],
			...
			n = [],
			r = void 0;
		if (a.addMoreShopsClicked) {
			var i = +a.$locator.dataset.shopCount;
			(r = i + a.loadMoreShopsIncrementAmount), (a.$locator.dataset.shopCount = r), (a.addMoreShopsClicked = !1);
		} else (a.$locator.dataset.shopCount = a.searchResultsAmount), (r = a.searchResultsAmount);
		a.dataSources.forEach(function (o) {
			o.clearMarkers(), n.push(o.search(e, t));
		}),
			o && s && (r = 1),
			Promise.all(n)
				.then(function (e) {
					...
					e.forEach(function (e, o) {
						e.hits.hits.forEach(function (e) {
							t.push({ dataSource: a.dataSources[o], data: e });
						});
					});
					...
					t
						...
						a.setDeviceMarkerBounds(s, n, o),
						o ? a.toggleResultsMessage(!1, o) : a.toggleResultsMessage(!0);
				})
				.catch(function (e) {
	...
	init: function (e, t) {
		var o = STCUK[t];
		o.setDomElements(e, t), o.showLoadMessage(), o.bindEvents();
	}
}),
	(STCUK.mapStyle = [
...
STCUK.shop = {
	...
	_setDomElements: function (t, e, n) {
		var o = STCUK[e];
		(o.map = t),
			(o.invocationName = e),
			(o.config = n),
			(o.searchUrl = t.$el.dataset.shopSearchUrl),
			(o.loadedMarkers = []),
			(o.markerIcon = n.markerIcon),
			(o.searchQuery = { query: { bool: { must: { term: { category: o._category } } } } }),
			o.loadAllMarkers(),
			(o.addMoreShopsClicked = !1),
			o.firstShopLatLng;
	},
	...
	addMarker: function (t, e, n, o) {
		...
		'number' == typeof e && a._addResultsPanel(t, e, i, r),
			i.addListener('click', function () {
				var t = i.getPosition().lat(),
					e = i.getPosition().lng();
				a.map.map.setZoom(15), a.map.map.setCenter(i.getPosition()), a.map.markerSearch(t, e);
			}),
			n && n.extend(i.position),
			a.loadedMarkers.push(i);
	},
	_toggleMoreButton: function (t) {
		var e = STCUK[this.invocationName],
			n = document.createElement('li');
		(n.innerHTML = '<button class="load-more" type="button">Load More</button>'),
			t.insertAdjacentElement('beforeend', n),
			t.querySelector('.load-more').addEventListener('click', e._getMoreShops.bind(e));
	},
	_showMoreShops: function (t, e, n, o) {
		var a = STCUK[this.invocationName];
		0 == t && ((a.map.firstShopLatLng.lat = n.lat), (a.map.firstShopLatLng.lng = n.lon)),
			t === e && 'true' == a.map.loadMoreShops && a._toggleMoreButton(o, a.map.firstShopLatLng);
	},
	...
	_addResultsPanel: function (t, e, n, o) {
		var a = STCUK[this.invocationName],
			r = a.map.$results,
			s = t._source,
			i = e,
			l = o - 1,
			c = r.querySelector('ul'),
		...
		if (r) {
			...
			(d.innerHTML =
				...
				a._getOpeningHoursForDay(s, new Date().getDay(), !0) +
				'\n\t\t\t\t\t\t\t\t</span>\n\t\t\t\t\t\t\t\t' +
				a._getOpeningHours(s) +
				'\n\t\t\t\t\t\t\t</p>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="more">\n\t\t\t\t\t\t\t<p><a href="tel:' +
				s.phone +
				'">Phone: ' +
				s.phone +
			...
			h.addEventListener('click', function (t) {
				...
				n.addListener('click', function () {
					var t = n.label.text,
						e = a.map.$results.querySelector('[data-shop-number="' + t + '"]').closest('li');
					a._scrollIntoView(e);
				}),
				a._scrollToTop(i, l);
		}
	...
	_scrollToTop: function (t, e) {
		var n = STCUK[this.invocationName];
		t === e && n.map.$results.scrollTo(0, 0);
	},
	_scrollIntoView: function (t) {
		var e = STCUK[this.invocationName],
			n = t.offsetTop - 10;
		e.map.$results.scrollTo({ top: n, left: 0, behavior: 'smooth' });
	},
	_getOpeningHours: function (t) {
		for (var e = STCUK[this.invocationName], n = '<ul class="hide">', o = 1; o < e.days.length; o++)
			n += e._getOpeningHoursForDay(t, o);
		return (n += e._getOpeningHoursForDay(t, 0)), (n += '</ul>');
	},
	_getOpeningHoursForDay: function (t, e, n) {
		var o = STCUK[this.invocationName],
			a = o.days[e];
		...
		return (
			t.openingHours.forEach(function (t) {
				t.day === o.days[e] && (r = t.openingTime + '-' + t.closingTime);
			}),
			n ? 'Today - ' + r : '<li>' + a + ' - ' + r + '</li>'
		);
	},
	...
	search: function (t, e) {
		var n = STCUK[this.invocationName],
			o = n.searchQuery,
			a = n.map.$locator.dataset.shopCount;
		return (
			(o.sort = [{ _geo_distance: { location: { lat: t, lon: e }, order: 'asc', unit: 'miles' } }]),
			fetch(n.searchUrl + '?size=' + a, {
				method: 'post',
				headers: { 'Content-Type': 'application/json' },
...
STCUK.paypal = {
	...
	createFormTmpRecord: function () {
		...
		(r.formdata.donation_method_acc = { donation_method_rbg: 'paypal' }),
			...
			fetch(t.paypalUrl, {
				method: 'post',
				headers: { 'X-PayPalReturnUrl': t.returnUrl, 'X-PayPalLogoImageUrl': t.logoImageUrl },
...
STCUK.paypalRedirect = {
	...
	pollForRecaptchaLoaded: function (e, a, t) {
		...
		t = t || 100;
		return new Promise(function a(i, n) {
			var r = e();
			r ? i(r) : Number(new Date()) < o ? setTimeout(a, t, i, n) : n(new Error('Re-captcha script not loading'));
		});
	},
	...
	setFormData: function (e, a) {
		var t = STCUK.utility,
			...
			n = a ? JSON.parse(sessionStorage.getItem('amountDetails')) : STCUK.utility.cookie.getItem('amountDetails');
		a ? sessionStorage.setItem('giftAidFlag', '' + i) : STCUK.utility.cookie.setItem('giftAidFlag', '' + i);
		var r = e.formdata;
		(r.header_acc = { above18_cb: 'true' }),
			(r.gift_aid_acc = { gift_aid_declaration: i }),
			(r['paypal-rg'] = { 'paypal-rg-token': o }),
			(r['paypal-rg'].readable_start_date = a
				? sessionStorage.getItem('readableStartDate') || ''
				: STCUK.utility.cookie.getItem('readableStartDate') || ''),
			(r.donation_amount_acc = n),
			r.address_billing_acc || (r.address_billing_acc = {}),
			(r.address_billing_acc.joinus_update_email = 'false'),
			(r.address_billing_acc.joinus_update_phone = 'false'),
			(r.address_billing_acc.joinus_update_post = 'false'),
			(r.address_billing_acc.joinus_update_sms = 'false'),
			(r.donation_method_acc = { donation_method_rbg: 'paypal-rg' }),
	...
	handlePaypalResponseMonthly: function () {
		var e = STCUK.paypalRedirect,
			...
			n = STCUK.utility.cookie.getItem('formMetadata'),
			r = void 0;
		if (o || !i || (a.isLegacyEdge() && !n)) e.showErrorMessage();
		else {
			STCUK.loader.show(e.paypalReturnMsg),
				i ? (e.setFormData(i, !0), (r = i)) : STCUK.utility.isLegacyEdge() && (e.setFormData(n, !1), (r = n)),
				e.runCaptchaPolling(r, 'regular');
		}
	},
					  ...
					  fetch(e.paypalUrl + '/getData/' + t, {
							...
							.then(function (o) {
								var i = o.formData,
									...
									c = e.metadata;
								(c.formdata = n),
									(c.formdata.address_billing_acc = {
										title: '',
									...
									(c.formdata.paypalResponse1 = r),
									(c.formdata.paypalResponse2 = s),
									(c.formdata.key = t),
									(c.formdata.marketing_preferences = { preference_submit: 'false' }),
									e.runCaptchaPolling(c, 'single');
							})
							.catch(function (a) {
		...
		i.fetchToken(function (i) {
			...
			new Promise(function (a, o) {
				...
				fetch(t.actionUrl, {
					...
					.then(function (e) {
						clearTimeout(i), r || a(e);
					})
					.catch(function (e) {
						console.error('submitFormPost failed! ', e), r || o(e);
					});
			})
				.then(function (e) {
					return e.json();
				...
				.then(function (i) {
					(i.formdata = e.formdata),
						...
						i.errorMessage ? t.goToPodErrorHandling(n, t.paymentType) : (window.location = i.thankYouPage),
				...
				.catch(function (a) {
					console.error(a),
						r &&
							...
							t.goToPodErrorHandling(n, t.paymentType, 2));
				});
		});
	},
	...
	bindEvents: function () {
		...
		window.addEventListener('resize', a),
			...
			window.addEventListener('DOMContentLoaded', function () {
				e.adjustHeight(), e.checkIfMenuWrapped(), e.navigationScroll();
			});
	},
...
STCUK.quizQuestion = {
	...
	setDomElements: function (e, t) {
		...
		(e.dataset.invocationName = t),
			...
			(i.isHidden = !0),
			(i.isAnswered = !1),
			(i.quizEl = e.closest('[data-role="quizContainer"]')),
			(i.quiz = STCUK[i.quizEl.dataset.invocationName]),
			(i.answers = e.querySelectorAll('[data-role="quizAnswer"]')),
			(i.pageHeader = document.querySelector('.ui-component.header')),
			(i.cookieHeader = document.querySelector('.cookie-container'));
	},
	hide: function (e) {
		for (var t = STCUK[this.invocationName], i = 0; i < t.answers.length; i++) {
			var n = t.answers.item(i),
				s = t.quiz.getSectionById(n.dataset.nextSectionId);
	...
	show: function (e, t) {
		var i = STCUK[this.invocationName];
		if ((i.el.classList.remove('hide'), (i.isHidden = !1), i.isAnswered)) {
			var n = i.getAnswer().dataset.nextSectionId,
				s = i.quiz.getSectionById(n);
			s ? s.show(i.getAnswer().dataset.noScroll, i) : 'true' !== e ? i.scrollIntoView() : t.scrollIntoView();
		} else 'true' !== e ? i.scrollIntoView() : t.scrollIntoView();
	},
	scrollIntoView: function () {
		var e = STCUK[this.invocationName],
			t = e.pageHeader.offsetHeight + (e.cookieHeader ? e.cookieHeader.offsetHeight : 0),
			i = e.el.offsetTop - t;
		window.scrollTo({ behavior: 'smooth', left: 0, top: i });
	},
	getAnswer: function () {
		for (var e = STCUK[this.invocationName], t = 0; t < e.answers.length; t++) {
			var i = e.answers.item(t);
			if (i.classList.contains('selected')) return i;
		}
	},
	setAnswer: function (e) {
		...
		for (var i = 0; i < t.answers.length; i++) {
			var n = t.answers.item(i);
			n === e
				? (n.classList.add('selected'), n.classList.remove('not_selected'))
				: (n.classList.remove('selected'), n.classList.add('not_selected'));
		}
	},
	...
	init: function (e, t) {
		var i = STCUK[t];
		i.setDomElements(e, t), i.bindEvents();
	}
};
STCUK.quizAnswer = {
	bindEvents: function () {
...
STCUK.teaserTwoColumn = {
	setDomElements: function (e, a) {
		var t = $(e);
		(this.$el = t),
			t.data('invocationName', a),
			(this.invocationName = a),
			(this.$teaserNextAvailable = this.$el.closest('.section').next().find('.teaser-two-column.ui-component'));
	},
	adjustSpacing: function () {
		var e = STCUK[this.invocationName];
		(e.teaserVariants = '.teaser-variant-two, .teaser-variant-one'),
			window.innerWidth < STCUK.utility.getBreakpoint('small-device') && e.$teaserNextAvailable
				? (e.$teaserNextAvailable.find(e.teaserVariants).addClass('teaser-negative-mobile'),
				  e.$el.find(e.teaserVariants).addClass('teaser-negative-mobile'))
				: e.$teaserNextAvailable.addClass('ui-two-column-negative');
	},
	init: function (e, a) {
		var t = STCUK[a];
		t.setDomElements(e, a), t.adjustSpacing();
	}
};
...
(STCUK.app = {
	bindEvents: function () {
		var t = $(document);
		t
			.on('mousedown', 'a', function (o) {
				return t.focus(), t.blur(), !1;
			})
			.on('mousedown', 'button', function (o) {
				return t.focus(), t.blur(), !1;
			}),
			$(window).on('unload', function () {
				window.paypalWindow && window.paypalWindow.close();
			}),
			$('.body-text.ui-component').on('click', 'a[href^=#]', function (t) {
				t.preventDefault();
				var o,
					e = $(this).attr('href'),
					n = 0;
				(e = e.length ? e.substring(1) : ''),
					(o = $('[name=' + e + '], [id=' + e + ']')).length &&
						((n = STCUK.utility.getHeaderHeight(1)), $('body,html').scrollTop(o.offset().top - n - 15));
			}),
			document.addEventListener('DOMContentLoaded', function () {
				var t = document.querySelector('.admin-page');
				window.IntersectionObserver && !t
					? yall({
							...
							events: {
								load: function (t) {
									t.target.classList.contains('lazyload') ||
										'IMG' != t.target.nodeName ||
										t.target.classList.add('lazy-loaded');
								}
							}
					  })
	...
	checkForFormsComponents: function () {
		var t = $(document.body).hasClass('donation-form-page');
		if (STCUK.componentLoader.formsComponents.length > 0 && !t) {
			var o = '?ts=' + Date.now();
			STCUK.utility.loadStyleSheet('/etc/clientlibs/ui/forms.css' + o),
				STCUK.utility.loadScript('/etc/clientlibs/ui/forms.js' + o);
		}
	},
	checkIfBlogPage: function () {
		var t = document.querySelector('#page-content');
		!!t && t.classList.contains('blog-page') && STCUK.blogPageUtils.checkForEmbeddedTweets();
	},
	init: function () {

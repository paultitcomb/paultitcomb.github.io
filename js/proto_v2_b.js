...
const $openSharePanelBtn = document.querySelector('.open-share-panel-btn');
...
function handleShareBtnClick(e) {
	if (supportsShareApi && isMobile()) {
		...
		$socialShareContainer.classList.toggle('open');
	}
}
...
function setShareIcon() {
	...
	const appleDevice = ua.includes('iPhone') || ua.includes('iPad') || ua.includes('Intel Mac OS');
	if (appleDevice) {
		$openSharePanelBtn.classList.add('apple-share-icon');
	} else {
		$openSharePanelBtn.classList.add('standard-share-icon');
	}
}
...
window.addEventListener('load', e => {
	setTimeout(() => {
		$socialShareContainer.classList.remove('off-screen');
	}, 1000);
});
